...
(cd $REPO && rm -rfv */target && git checkout trunk && mvn clean && git checkout . && git pull && git checkout $USETAG && mvn dependency:tree && mvn dependency:tree > dependencies.txt && mvn clean install -DskipTests && git log -n1 > version.txt)
