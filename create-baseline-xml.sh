...
(cd $REPO && git checkout trunk && git checkout . && git pull && git checkout $USETAG && mvn clean install -DskipTests && mvn dependency:tree && mvn dependency:tree > dependencies.txt && mvn clean && git log -n1 > version.txt)
